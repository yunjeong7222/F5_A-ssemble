        ...
        const load = async () => {
            try {
                ...
                if (isBookmarked) {
                    ...
                    wfData = res.data.data;
                    resultJson =
                        typeof wfData.result_json === 'string' ? JSON.parse(wfData.result_json) : wfData.result_json;
                }
                                    ...
                                    <div key={i} style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                        ...
                                        >
                                            <img
                                                src={
                                                    tool.thumbnail ||
                                                    `https://www.google.com/s2/favicons?domain=${tool.tool_name?.toLowerCase().replace(/\s/g, '')}.com&sz=64`
                                                }
                                                alt={tool.tool_name}
                            ...
                            {data.resultJson?.steps?.map((step, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        style={{
                                            border: `1px solid ${isEditing ? '#c7d2fe' : '#e2e8f0'}`,
                                            borderRadius: '12px',
                                            padding: '16px',
                                            marginBottom: '12px',
                                            ...
                                            {isEditing ? (
                                                ...
                                                <>
                                                    ...
                                                    <button
                                                        onClick={() =>
                                                            navigator.clipboard
                                                                .writeText(editedPrompts[stepKey] || '')
                                                                .then(() => alert('복사됐습니다!'))
                                                        }
                                                        style={{
                                                            position: 'absolute',
...
const WorkflowSection = ({myWorkflows, setMyWorkflows, myPosts, setMyPosts, workflowSubTab, setWorkflowSubTab}) => {
    // ✅ 수정: 내부 state 제거 — MyPage의 activeTab을 workflowSubTab prop으로 받아서 사용
    // const [workflowSubTab, setWorkflowSubTab] = useState('mine'); // ❌ 삭제
    ...
    // ✅ 수정: activeTab 값을 내부 subTab 값으로 매핑
    // MyPage: 'workflows' → 내 워크플로우, 'bookmarks' → 북마크, 'posts' → 내가 쓴 글
    ...
    return (
        <div className="mypage-workflow-section">
            ...
            {/* ✅ 수정: 내부 탭 버튼 제거 — 사이드바에서 직접 탭 전환하므로 불필요 */}
            {/* 
      <div className="mypage-workflow-filter">
        <button onClick={() => setWorkflowSubTab('mine')} ...>내 워크플로우</button>
        <button onClick={() => setWorkflowSubTab('bookmarked')} ...>북마크</button>
        <button onClick={() => setWorkflowSubTab('posts')} ...>내가 쓴 글</button>
      </div>
      */}
