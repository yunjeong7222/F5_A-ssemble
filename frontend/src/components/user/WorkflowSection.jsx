...
const WorkflowDetailModal = ({workflowId, isBookmarked, onClose}) => {
        ...
        const load = async () => {
            try {
                ...
                if (isBookmarked) {
                    ...
                    wfData = res.data.data;
                    resultJson =
                        typeof wfData.result_json === 'string'
    ...
    const handleSave = async () => {
        ...
        if (!hasChanged) {
            Alert.fire({text: '변경된 내용이 없습니다.', showConfirmButton: false, timer: 1500});
            return;
        }
        ...
        try {
            ...
            setIsEditing(false);
            Alert.fire({text: '저장되었습니다!', showConfirmButton: false, timer: 1500});
        } catch (err) {
            console.error('저장 실패:', err);
            Alert.fire({text: '저장에 실패했습니다.', showConfirmButton: false, timer: 1500});
        } finally {
            setIsSaving(false);
        }
    };
                        ...
                        <div style={{overflowY: 'auto', flex: 1, paddingRight: '4px'}}>
                            {data.resultJson?.steps?.map((step, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        style={{border: `1px solid ${isEditing ? '#c7d2fe' : '#e2e8f0'}`, borderRadius: '12px', padding: '16px', marginBottom: '12px', transition: 'border-color .2s'}}
                                    >
                                        ...
                                        <div style={{background: '#0f172a', borderRadius: '8px', padding: '12px', position: 'relative'}}>
                                            {isEditing ? (
                                                <textarea
                                                    ...
                                                    style={{width: '100%', minHeight: '75px', background: 'transparent', border: 'none', outline: 'none', color: '#f8fafc', fontSize: '12px', lineHeight: '1.6', fontFamily: 'Pretendard, sans-serif', resize: 'vertical', boxSizing: 'border-box'}}
                                                />
                                            ) : (
                                                <>
...
// WorkflowSection
// ─────────────────────────────────────────
const WorkflowSection = ({myWorkflows, setMyWorkflows, myPosts, setMyPosts, workflowSubTab, setWorkflowSubTab}) => {
    const [selectedWorkflow, setSelectedWorkflow] = useState(null);
    const [selectedPostId, setSelectedPostId] = useState(null);
    const [bookmarkedWorkflows, setBookmarkedWorkflows] = useState([]);
    const navigate = useNavigate();

    // MyPage activeTab 값 → 내부 subTab 값 매핑
    const subTab = workflowSubTab === 'bookmarks' ? 'bookmarked' : workflowSubTab === 'posts' ? 'posts' : 'mine';
    ...
    const handleDeleteWorkflow = async (id) => {
        ...
        try {
            ...
            Alert.fire({title: '삭제에 실패했습니다.', showConfirmButton: false, timer: 1500});
        }
    };
    ...
    const visibleWorkflows = subTab === 'mine' ? myWorkflows : bookmarkedWorkflows;
    ...
    return (
        <div className="mypage-workflow-section">
            ...
            {/* 빈 상태 */}
            {subTab !== 'posts' && visibleWorkflows.length === 0 && (
                <div style={{textAlign: 'center', padding: '120px 0', color: '#94a3b8', fontSize: '14px'}}>
                    {subTab === 'mine' ? '생성한 워크플로우가 없습니다.' : '저장한 워크플로우가 없습니다.'}
                </div>
            )}
            {subTab === 'posts' && myPosts.length === 0 && (
                <div style={{textAlign: 'center', padding: '100px 0', color: '#94a3b8', fontSize: '14px'}}>
                    작성한 글이 없습니다.
                </div>
            )}
            ...
            {subTab !== 'posts' &&
                visibleWorkflows.map((wf) => (
                    <div
                        ...
                        onClick={() => setSelectedWorkflow({id: wf.id, isBookmarked: subTab === 'bookmarked'})}
                        style={{cursor: 'pointer'}}
                    >
                        <div className="mypage-workflow-card-header">
                            <span className="mypage-workflow-label">{wf.title}</span>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
            ...
            {subTab === 'posts' && (
                <div style={{display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px'}}>
                    {myPosts.map((post) => (
                        ...
                        >
                            {post.thumbnail_url && (
                                <img src={post.thumbnail_url} alt={post.title} style={{width: '140px', height: '80px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0}}/>
                            )}
