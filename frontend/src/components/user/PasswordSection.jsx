...
export default function PasswordSection() {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      setErrorMessage(
        err.response?.data?.message || '현재 비밀번호가 틀렸거나 변경에 실패했습니다.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="password-section">
      <form onSubmit={handleUpdatePassword}>
        ...
        <div className="password-form-group">
          ...
          />
          {currentStatus === 'error' && (
            <p className="password-error-text">{errorMessage}</p>
          )}
        </div>
        ...
        <div className="password-form-group password-form-group--last">
          ...
          {confirmPassword && newPassword === confirmPassword && newPassword.length >= 8 && (
            <p className="password-match-text password-match-text--ok">
              비밀번호가 일치합니다.
            </p>
          )}
        </div>
