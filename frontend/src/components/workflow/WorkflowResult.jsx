...
const WorkflowResult = ({ workflowResult }) => {
  ...
  const toolDirectory = {
    ...
    'Claude': { domain: 'anthropic.com', category: '기획 · 아이디어', desc: '뛰어난 문장력으로 대본 및 아이디어를 기획합니다.' }, // ⭐ Claude 추가
    'Gemini': { domain: 'gemini.google.com', category: '기획 · 아이디어', desc: '강력한 AI 모델로 아이디어를 기획하고 분석합니다.' }, // ⭐ Gemini 추가
  ...
  // ⭐ 핵심 수정 부분: (toolName)만 받던 것을 (toolName, stepCategory)를 받도록 변경
  const getToolInfo = (toolName, stepCategory) => {
    // 1. 프론트엔드에 등록된 툴이면 해당 정보를 반환
    if (toolDirectory[toolName]) {
      return toolDirectory[toolName];
    }
    
    // 2. 등록되지 않은 새로운 툴이더라도, 백엔드에서 넘겨준 카테고리(stepCategory)를 그대로 살려줌
    // 이렇게 하면 '알 수 없음'이 뜨지 않고 해당 카테고리에 맞는 애니메이션 박스가 정상적으로 나옵니다!
    return { 
      domain: 'google.com/search?q=' + toolName, 
