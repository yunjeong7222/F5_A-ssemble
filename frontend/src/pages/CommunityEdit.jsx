      ...
      <div className="write-container">
        ...
        <section className="write-section">
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            <button onClick={() => setUploadType('file')} className={`comm-filter-btn ${uploadType === 'file' ? 'active' : ''}`}>이미지 / 영상</button>
            <button onClick={() => setUploadType('youtube')} className={`comm-filter-btn ${uploadType === 'youtube' ? 'active' : ''}`}>유튜브 URL</button>
          </div>
          ...
          {uploadType === 'file' && (
            <>
              ...
              >
                {previewUrl ? (
                  <>
                    {(selectedFile?.type || existingMediaType) === 'video' || selectedFile?.type?.startsWith('video/') ? (
          ...
          {uploadType === 'youtube' && (
            <div>
              <input type="text" value={youtubeUrl}
                ...
                className="write-input" style={{ marginBottom: 12 }} />
